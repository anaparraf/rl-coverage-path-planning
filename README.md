...
Para **treinar** um agente com PPO para um ambiente 5x5, 3 obstáculos, máximo de 200 passos e 500.000 timesteps (com observação 5x5):

```bash
python train_grid_world_cpp.py train 5 3 200 500000 --obs-window 5
```

Para **testar** o agente treinado em 100 episódios em um ambiente 5x5 com 3 obstáculos:

```bash
python train_grid_world_cpp.py test 5 3 --model-path data/SEU_MODELO.zip --obs-window 5
```

Para **visualizar** o agente treinado em um único episódio em um ambiente 5x5 com 3 obstáculos:

```bash
python train_grid_world_cpp.py run 5 3 --model-path data/SEU_MODELO.zip --obs-window 5
```
...
```bash
python train_grid_world_cpp.py curriculum 10 12 400 500000 --model-path data/SEU_MODELO_5x5.zip --obs-window 5
```

Neste caso, você deve informar o modelo inicial da rede (por exemplo, um modelo pré-treinado no 5x5) usando o argumento `--model-path`.

### APS08 (instruções completas)

Consulte o arquivo `APS08_README.md` para instruções completas de treinamento, avaliação, geração de gráficos, GIF e tabela automática de métricas.
